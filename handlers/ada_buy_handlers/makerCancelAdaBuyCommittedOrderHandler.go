...
// MakerCancelAdaBuyCommittedOrderHandler cancels a committed ADA buy order by the maker.
func MakerCancelAdaBuyCommittedOrderHandler(c *fiber.Ctx) error {
	...
	cancelFee, err := utility.CalculateFee(u.Precision, u.OrderThreshold, u.MinOrderAmount, u.CancelMinFee, u.CancelPct, u.OrderAmount)
	if err != nil {
		log.Println(err)
		return err
	}
	...
	cborString, txHash, err := ada_p2p_buy.MakerCancelCommittedOrder(orderInfo, treasuryInfo, config.GetAdaP2PBuyAdminWallet())
