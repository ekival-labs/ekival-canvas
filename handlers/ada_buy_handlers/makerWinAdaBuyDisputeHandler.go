...
// MakerWinAdaBuyDisputeHandler handles the case when maker wins a dispute on ADA buy order.
func MakerWinAdaBuyDisputeHandler(c *fiber.Ctx) error {
	...
	makerWinDisputeRedeemer := &model.MakerWinDisputeRedeemer{}

	cborString, txHash, err := ada_p2p_buy.MakerWinDispute(orderInfo, treasuryInfo, makerWinDisputeRedeemer, config.GetAdaP2PBuyAdminWallet())
