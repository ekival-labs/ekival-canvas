...
// // MakerDeadline is less than TakerDeadline in Hours for example 4 day = 4 * 24 * 60 * 60 * 1000
// 	var md int64 = currentTime + (MakerDeadline * 1000)
// // TakerDeadline is whole Order Time in Hours for example 5 days = 5 * 24 * 60 * 60 * 1000
...
// cborString, txHash, err := ada_p2p_sell.MakerCreateOrder(orderInfo, config.GetAda_SSTAdminWallet())
func MakerCreateOrder(order *model.Order, adminWallet *config.Wallet) (string, string, error) {
	...
	makerCommittingAmount := order.OrderTxInfo.MakerFee + order.OrderTxInfo.CollateralAmount
	...
	apolloBE, err = apolloBE.
		...
		PayToContract(
			order.OrderTxInfo.EscrowContractAddress, orderDatumMarshaled, int(makerCommittingAmount), true, apollo.Unit{
				PolicyId: order.OrderTxInfo.StateTokenPolicyId,
				Name:     order.OrderInfo.OrderId,
				Quantity: int(1),
			},
