...
// cborString, txHash, err := ada_p2p_sell.MakerCancelUncommittedOrder(orderInfo, config.GetAda_P2PSellAdminWallet())
func MakerCancelUncommittedOrder(order *model.Order, adminWallet *config.Wallet) (string, string, error) {
	...
	log.Info().
		...
		Msg("Maker uncommitted order canceled successfully")
