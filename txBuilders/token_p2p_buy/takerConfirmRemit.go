...
// cborString, txHash, err := token_p2p_buy.TakerConfirmRemit(orderInfo, config.GetEki_P2PBuyAdminWallet())
func TakerConfirmRemit(order *model.Order, adminWallet *config.Wallet) (string, string, error) {
	...
	// Calculate totalAmount - for token trades, this is the token quantity
	// TakerConfirmRemit only changes state, the token amount stays constant as OrderAmount
