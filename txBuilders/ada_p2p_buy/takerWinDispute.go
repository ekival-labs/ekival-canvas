...
// cborString, txHash, err := ada_p2p_buy.TakerWinDispute(orderInfo, treasuryInfo, takerWinDisputeRedeemerUnmarshaled, config.GetAda_P2PBuyAdminWallet())
...
func TakerWinDispute(order *model.Order, treasury *model.TreasuryInfo, disputeRedeemer *model.MakerWinDisputeRedeemer, adminWallet *config.Wallet) (string, string, error) {
	...
	payToAdaval := order.OrderTxInfo.MakerFee + order.OrderTxInfo.TakerFee + order.OrderTxInfo.DisputeFee
	...
	log.Trace().
		...
		Int64("pay_to_adaval", payToAdaval).
	...
	log.Debug().
		...
		Int64("pay_to_adaval", payToAdaval).
		Msg("Payment amounts calculated for taker win dispute")
	...
	apolloBE, err = apolloBE.
		...
		PayToContract(
			treasury.Address, treasury.Datum, int(payToAdaval), true,
		).
	...
	log.Info().
		...
		Int64("dispute_penalty", disputeRedeemer.DisputeInfo.DisputePenalty).
		Msg("Dispute resolved successfully, taker wins")
