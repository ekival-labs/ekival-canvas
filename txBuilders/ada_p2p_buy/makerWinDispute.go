...
// cborString, txHash, err := ada_p2p_buy.MakerWinDispute(orderInfo, treasuryInfo, makerWinDisputeRedeemerUnmarshaled, config.GetAda_P2PBuyAdminWallet())
...
func MakerWinDispute(order *model.Order, treasury *model.TreasuryInfo, disputeRedeemer *model.MakerWinDisputeRedeemer, adminWallet *config.Wallet) (string, string, error) {
	...
	log.Trace().
		Str("function", "MakerWinDispute").
		Str("order_id", order.OrderInfo.OrderId).
		Interface("dispute_redeemer", disputeRedeemer).
		Msg("Marshaling dispute redeemer to Plutus")

	makerWinDisputeRedeemerMarshaled, err := plutusEncoder.MarshalPlutus(*disputeRedeemer)
	if err != nil {
		log.Error().
			Err(err).
			Str("function", "MakerWinDispute").
			...
			Str("order_id", order.OrderInfo.OrderId).
			Msg("Failed to marshal dispute redeemer to Plutus")
		return "", "", err
	}
	...
	log.Debug().
		Str("function", "MakerWinDispute").
		Str("order_id", order.OrderInfo.OrderId).
		Msg("Dispute redeemer marshaled successfully")
	...
	payToMakerAmount := order.OrderTxInfo.CollateralAmount + order.OrderTxInfo.MakerFee + order.OrderInfo.OrderAmount + disputeRedeemer.DisputeInfo.DisputePenalty
	payToTakerAmount := order.OrderTxInfo.CollateralAmount + order.OrderTxInfo.TakerFee - disputeRedeemer.DisputeInfo.DisputePenalty - order.OrderTxInfo.DisputeFee
	...
	log.Trace().
		...
		Msg("Calculated payment amounts for dispute resolution")

	log.Debug().
		Str("function", "MakerWinDispute").
		Str("order_id", order.OrderInfo.OrderId).
		Int64("pay_to_maker", payToMakerAmount).
		Int64("pay_to_taker", payToTakerAmount).
		Int64("dispute_fee", order.OrderTxInfo.DisputeFee).
		Msg("Payment amounts calculated for maker win dispute")
	...
	log.Info().
		Str("function", "MakerWinDispute").
		Str("order_id", order.OrderInfo.OrderId).
		Msg("Building dispute resolution transaction with Apollo")
	...
	if err != nil {
		log.Error().
			...
			Str("order_id", order.OrderInfo.OrderId).
			Msg("Failed to build dispute resolution transaction")
		return "", "", err
	}
	...
	log.Info().
		...
		Int64("dispute_penalty", disputeRedeemer.DisputeInfo.DisputePenalty).
		Msg("Dispute resolved successfully, maker wins")
