...
func MakerCreateOrder(order *viewmodel.Order, adminWallet *config.AdminWallet) (string, string, error) {
	...
	collateralAmount := utility.CalculateFee(
		order.BrokerageInfo.Precision,
		order.OrderInfo.OrderAmount,
		order.BrokerageInfo.OrderThreshold,
		order.BrokerageInfo.CollateralPct,
		order.BrokerageInfo.MinCollateral,
	)
