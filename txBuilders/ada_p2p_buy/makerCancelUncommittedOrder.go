...
// cborString, txHash, err := ada_p2p_buy.MakerCancelUncommittedOrder(orderInfo, config.GetAda_P2PBuyAdminWallet())
func MakerCancelUncommittedOrder(order *model.Order, adminWallet *config.Wallet) (string, string, error) {
	log.Trace().
		...
		Msg("Function entry: Canceling uncommitted maker order")
	...
	defer func() {
		if r := recover(); r != nil {
			...
			log.Panic().
				...
				Str("function", "MakerCancelUncommittedOrder").
				Str("order_id", order.OrderInfo.OrderId).
				Msg("Panic occurred in MakerCancelUncommittedOrder")
			panic(r)
		}
	}()
	...
	log.Trace().
		...
		Msg("Calculated maker committed amount for cancellation")
	...
	log.Debug().
		...
		Interface("user_utxos", userUtxos).
	...
	log.Trace().
		...
		Msg("Transaction retrieved from Apollo backend")
	...
	log.Trace().
		...
		Str("order_id", order.OrderInfo.OrderId).
		Str("tx_hash", hex.EncodeToString(txHash)).
		Str("tx_bytes_hex", hex.EncodeToString(txByte)).
	...
	log.Debug().
		...
		Interface("evaluation_result", evaluationResult).
	...
	log.Info().
		...
		Str("tx_cbor", cbor).
		Msg("Uncommitted order canceled successfully")
