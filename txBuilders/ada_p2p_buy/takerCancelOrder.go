...
// cborString, txHash, err := ada_p2p_buy.TakerCancelOrder(orderInfo, treasuryInfo, config.GetAda_P2PBuyAdminWallet())
func TakerCancelOrder(order *model.Order, treasury *model.TreasuryInfo, adminWallet *config.Wallet) (string, string, error) {
	log.Trace().
		Str("function", "TakerCancelOrder").
		...
		Msg("Function entry: Canceling order by taker")
	...
	defer func() {
		if r := recover(); r != nil {
			...
			log.Panic().
				...
				Str("function", "TakerCancelOrder").
				Str("order_id", order.OrderInfo.OrderId).
				Msg("Panic occurred in TakerCancelOrder")
			panic(r)
		}
	}()
	...
	payToMakerAmount := order.OrderInfo.OrderAmount + order.OrderTxInfo.MakerFee + order.OrderTxInfo.CollateralAmount + order.BrokerageInfo.CancelPenalty
	payToTakerAmount := order.OrderTxInfo.TakerFee + order.OrderTxInfo.CollateralAmount - order.BrokerageInfo.CancelPenalty - order.OrderTxInfo.CancelFee
	...
	log.Trace().
		...
		Msg("Calculated payment amounts for taker cancellation")
	...
	log.Info().
		Str("function", "TakerCancelOrder").
		Str("order_id", order.OrderInfo.OrderId).
		Msg("Building cancellation transaction with Apollo")
	...
	if err != nil {
		log.Error().
			...
			Str("order_id", order.OrderInfo.OrderId).
			Msg("Failed to build cancellation transaction")
		return "", "", err
	}
	...
	log.Info().
		...
		Msg("Order canceled successfully by taker")
