...
// cborString, txHash, err := ada_p2p_buy.MakerCancelCommittedOrder(orderInfo, treasuryInfo, config.GetAda_P2PBuyAdminWallet())
...
func MakerCancelCommittedOrder(order *model.Order, treasury *model.TreasuryInfo, adminWallet *config.Wallet) (string, string, error) {
	...
	payToMakerAmount := order.OrderInfo.OrderAmount + order.OrderTxInfo.MakerFee + order.OrderTxInfo.CollateralAmount - order.BrokerageInfo.CancelPenalty - order.OrderTxInfo.CancelFee
	payToTakerAmount := order.OrderTxInfo.TakerFee + order.OrderTxInfo.CollateralAmount + order.BrokerageInfo.CancelPenalty
	...
	log.Trace().
		...
		Msg("Calculated payment amounts for cancellation")
	...
	log.Info().
		...
		Msg("Committed order canceled successfully")
