...
// cborString, txHash, err := token_p2p_sell.TakerConfirmReceipt(orderInfo, config.GetEki_P2PSellAdminWallet())
	...
	apolloBE, err = apolloBE.
		...
		PayToContract(
			...
			apollo.Unit{
				...
				Quantity: int(ekivalFee),
			},
		).
		PayToAddress(
			order.OrderInfo.MakerAddress,
			int(order.BrokerageInfo.AdaCollateral+(order.BrokerageInfo.AdaCollateral/2)),
			apollo.Unit{
				PolicyId: order.OrderInfo.TradeTokenPolicyId,
				Name:     order.OrderInfo.TradeTokenName,
