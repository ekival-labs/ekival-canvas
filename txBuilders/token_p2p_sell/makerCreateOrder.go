...
// cborString, txHash, err := token_p2p_sell.MakerCreateOrder(orderInfo, config.GetEki_SSTAdminWallet())
func MakerCreateOrder(order *model.Order, adminWallet *config.Wallet) (string, string, error) {
	...
	apolloBE, err = apolloBE.
		...
		PayToContract(
			...
			apollo.Unit{
				PolicyId: order.OrderInfo.TradeTokenName,
				Name:     order.OrderInfo.TradeTokenPolicyId,
